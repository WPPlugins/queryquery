...
<tbody>
<?php 
function queryquery_posttypes(){
    $queryquery_postypes_list = array();
    array_push($queryquery_postypes_list,"any");
    foreach ( get_post_types( '', 'names' ) as $post_type ) {
       array_push($queryquery_postypes_list,$post_type);
    }
    return $queryquery_postypes_list;
}
	...
	<tr class="<?php if ($evenodd % 2 == 0) {echo "even";}else{echo "alternate odd";} ?>">
        ...
        <td class="desc">
    		<select multiple="multiple" name="<?php echo $value.'[]' ?>" id="<?php echo $value ?>"><?php
				...
				$option_list_name["name"] == "posttype" ? $options_orderby =  queryquery_posttypes() : "";
                $select_orderby = get_option($value);
                foreach ($options_orderby as $option_orderby) {
                    ...
                    if ($option_list_name["name"] == "categorynumbers" || $option_list_name["name"] == "anticategorynumbers" ){
                        echo '<option value="' . $option_orderby_value . '" id="'. $option_orderby .'"'. $selected.'>'. get_cat_name($option_orderby). '</option>';
                    }else{
                         echo '<option value="' . $option_orderby_value . '" id="'. $option_orderby .'"'. $selected.'>'.$option_orderby. '</option>';
                    }
             }?></select>
