...
//Negotiates Post Types
		count($posttype) > 0 && is_array($posttype) ? $posttype = implode(",",$posttype) : "";
		...
		$finalposttype !== "" ? $args['post_type'] = $posttype : "";
		// debug posttype negotiation 
		if ($debugmode > 0){ echo "<!-- QueryQueryDebug : Normal Post Type = ".$posttype."-->";};
		if ($debugmode > 0){ echo "<!-- QueryQueryDebug : Final Post Types = ".json_encode($finalposttype)."-->";};
...
//create query url to view all
	if ($disablequeryurl <= 0 ){ 
	 ...
	 $queryurl = get_site_url()."?"."post_status=".$poststatus;
	  $finalAtts['post_type'] && $finalAtts['post_type'] !=="null" ? $queryurl .= "&post_type=".$finalAtts['post_type']: "";
	...
	if ($debugmode > 0){ echo "<!-- QueryQueryDebug : Query URL = ".$queryurl."-->";};
